...
def readmysql():  # 读取数据库
    ...
    textlist = []
    userlist = []
    conn = pymysql.connect(host='127.0.0.1', user='root', password='Your password', charset="utf8")  # 连接服务器
    with conn:
        cur = conn.cursor()
        cur.execute("SELECT * FROM nlp.masu WHERE id < '%d'" % 10000)
        #cur.execute("SELECT * FROM nlp.55kai WHERE id < '%d'" % 10000)
        rows = cur.fetchall()
        for row in rows:
            row = list(row)
            del row[0]
...
def wordtocloud(textlist):
    ...
    cloud = WordCloud(font_path='font.ttf',  # Chinese Characters
                      background_color="white",
                      max_words=2000,
                      mask=back_coloring,
                      max_font_size=100,
                      random_state=42,
                      width=1000, height=860, margin=2,  # background images
                      )
    for li in textlist:
        fulltext += ' '.join(jieba.cut(li, cut_all=False))
    wc = cloud.generate(fulltext)
    image_colors = ImageColorGenerator(back_coloring)
    plt.figure("55kaiXiaoxiao-word2c")
